...
const progressModifierCode = `export const progressModifier = \`
  ...
  float easeInOut(float progress) {
    return cubicBezier(progress, 0.0, 0.0, 1.0, 1.0);
  }

  float progressModifier(vec3 origin, vec3 target, float progress){
    float newProgress = easeInOut(progress);
    return newProgress;
  }
\`;
`
...
const Particles: React.FC<SceneContentProps> = ({
  ...
  useFrame(({ clock }) => {
    ...
    // since we are using uTime in vertex modifier, we need to update it
...
const noisyProgressFiles: Record<string, SandpackFile> = {
...
export default noisyProgressFiles
